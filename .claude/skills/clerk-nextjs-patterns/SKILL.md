...
| Auth not working on API routes | Missing matcher | Add `/api(.*)` and `/trpc(.*)` to `proxy.ts` (Next.js <=15: `middleware.ts`) |
| Cache returns wrong user's data | Missing userId in key | Include `userId` in `cache` key |
